...
async function loadConversationHistory() {
  ...
  await fetch(`/messages?conversation_id=${conversationId}`, {
    ...
    data.messages.forEach((msg) => {
      const senderType = msg.type === "ASSISTANT_MESSAGE"
        ? "assistant"
...
async function resolveConversation() {
  ...
  if (
    confirm(
      "Are you sure you want to mark this conversation as resolved? This action cannot be undone.",
    )
  ) {
    try {
      ...
      if (data.status === "resolved") {
        ...
        appendMessage(
          "system",
          "✅ Conversation marked as resolved successfully!",
        );
      ...
      appendMessage(
        "system",
        "❌ Failed to mark conversation as resolved. Please try again.",
      );
    }
  }
}
